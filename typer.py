...
async def start(proj_name, project_dir, pause, typo, s):
    ...
    for root, dirs, files in os.walk(project_dir):
        for i in files:
            ...
            local_file = local_dir + f'/{i}'
            try: local_file = local_file.replace('//', '/')                         #Fixes double slash on some local files, there is a better fix but its 12:09AM
            except: pass
            ...
            time.sleep(random.randint(3, 10)/10)
            nanoSaveMacro()
            #except STignore: print(f"STignoring: {i}")
    return
...
async def typeLines(lines, pause, typo, s):
    ...
    for line in lines:
        line = line.replace('\n', '')
        words = line.split(' ')
        for w in words:                                                                  #Write to terminal word by word
            chance_change_speed = random.randint(1,3)#Needs work
            if(chance_change_speed == 2):
                speed = random.uniform(speed * .85, speed * 1.15)                  #Vary the speed by 15% up or down, 33% chance per word
